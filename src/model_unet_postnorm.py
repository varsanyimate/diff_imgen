...
class Attention(nn.Module):
    """
    Multi-Head Self-Attention module for 2D feature maps.
    
    Implements standard dot-product attention with multi-head mechanism.
    ...
    def __init__(self, dim, heads=4, dim_head=32):
        super().__init__()
        # Scaling factor to prevent large dot product values and potential gradient issues
        self.scale = dim_head**-0.5
        self.heads = heads
        
        # Total hidden dimension across all heads
        hidden_dim = dim_head * heads
        
        # 1x1 convolution to generate query, key, and value projections
        # Uses no bias to reduce parameters
        self.to_qkv = nn.Conv2d(dim, hidden_dim * 3, 1, bias=False)
        
        # 1x1 convolution to combine multi-head outputs
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: Attention-processed feature map
        """
        b, c, h, w = x.shape  # Batch size, channels, height, width
        
        # Generate query, key, value projections
        # Splits the projection into 3 equal parts along channel dimension
        qkv = self.to_qkv(x).chunk(3, dim=1)  # Each part: (b, heads * dim_head, h, w)
        
        # Reshape projections to separate heads
        q, k, v = [
            t.reshape(b, self.heads, -1, h * w)  # Reshape to (batch, heads, dim_head, spatial)
            for t in qkv
        ]
        ...
        # Transpose to enable matrix multiplication
        sim = torch.matmul(q.transpose(-1, -2), k)  # Shape: (b, heads, spatial, spatial)
        ...
        # Final projection to original channel dimension
        return self.to_out(out)

class LinearAttention(nn.Module):
    """
    Linear Attention module - a more computationally efficient attention variant.
    
    Approximates standard attention with linear complexity operations.
    ...
    def forward(self, x):
        ...
        # Reshape projections to separate heads
        q = qkv[0].reshape(b, self.heads, -1, h * w)
        k = qkv[1].reshape(b, self.heads, -1, h * w)
        v = qkv[2].reshape(b, self.heads, -1, h * w)
        
        # Softmax and scaling
        q = q.softmax(dim=-2)  # Row-wise softmax on queries
        k = k.softmax(dim=-1)  # Column-wise softmax on keys
        q = q * self.scale
        
        # Compute context and output
        e = v.shape[2]  # Feature dimension
        
        # Compute key * value: aggregates value information
        context = torch.matmul(k, v.transpose(-2, -1))
        
        # Compute context * query: applies attention weights
        out = torch.matmul(context.transpose(-2, -1), q)
        
        # Reshape to original spatial dimensions
        out = out.reshape(b, self.heads * e, h, w)
        
        return self.to_out(out)

class PostNorm(nn.Module):
    """
    Post-Normalization wrapper for neural network modules.
    
    Applies group normalization after passing through a given function.
    
    Args:
        dim (int): Channel dimension for normalization
        fn (callable): Function/module to be wrapped
    """
    def __init__(self, dim, fn):
        super().__init__()
        self.fn = fn  # Wrapped function/module
        self.norm = nn.GroupNorm(1, dim)  # Group normalization
    
    def forward(self, x):
        """
        Apply wrapped function first, then normalization.
        
        Args:
            x (torch.Tensor): Input tensor
        
        Returns:
            torch.Tensor: Transformed and normalized tensor
        """
        x = self.fn(x)  # Apply wrapped function
    ...
    def __init__(
        ...
        for i in range(len(dims) - 1):
            ...
            down_blocks = nn.ModuleList([
                ...
                Residual(PostNorm(dim_out, LinearAttention(dim_out))),
        ...
        self.mid_attn = Residual(PostNorm(mid_dim, Attention(mid_dim)))
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
            ...
            up_blocks = nn.ModuleList([
                ...
                Residual(PostNorm(dim_in, LinearAttention(dim_in))),
