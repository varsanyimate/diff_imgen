...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, dim, dim_out, *, time_emb_dim=None, groups=8):
        ...
        self.mlp = (
            nn.Sequential(nn.ReLU(), nn.Linear(time_emb_dim, dim_out))
...
class ConvNextBlock(nn.Module):
    ...
    def __init__(self, dim, dim_out, *, time_emb_dim=None, mult=2, norm=True):
        ...
        # Optional time embedding processing
        self.mlp = (
            nn.Sequential(nn.ReLU(), nn.Linear(time_emb_dim, dim))
            if time_emb_dim is not None
            else None
        )
...
class PreNorm(nn.Module):
    """
    Pre-Normalization wrapper for neural network modules.
    
    Applies group normalization before passing through a given function.
    ...
    def forward(self, x):
        """
        Apply normalization before calling the wrapped function.
        ...
        return self.fn(x)  # Apply wrapped function
